...
public class StringTest
{
	...
	public void stringtestDecode()
	{
		...
		string s5 = "no-cache";
		if (StringTool.Decode(d1,100000,0) != s1 |
		    StringTool.Decode(d2,100000,0) != s2 |
		    StringTool.Decode(d3,100000,0) != s3 |
		    StringTool.Decode(d4,100000,0) != s4 |
		    StringTool.Decode(d5,100000,0) != s5)
		{
	...
	private static bool VerifyArray(byte[]? data1, byte[]? data2)
	{
		...
		if ((data1 == null && data2 != null) || (data1 != null && data2 == null) || data1.Length != data2.Length)
		{
			return false;
		}
		try
